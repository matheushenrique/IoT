...
class InterfaceController: WKInterfaceController,CLLocationManagerDelegate {
    ...
    override func awake(withContext context: Any?) {
        ...
        locman.desiredAccuracy = kCLLocationAccuracyBest // You can change the locaiton accuary here.
        ...
        print(localizacao)
        let m = SCMessage(code: SCCodeValue(classValue: 0, detailValue: 02)!, type: .confirmable, payload: localizacao.data(using: String.Encoding.utf8))
...
//        Mkpolyli
//        let path = MK (coordinates: &coordinates, count: coordinates.count)
    ...
    func swiftCoapClient(_ client: SCClient, didReceiveMessage message: SCMessage) {
        ...
        if let pay = message.payload {
            if let string = NSString(data: pay as Data, encoding:String.Encoding.utf8.rawValue) {
                ...
                destino =  CLLocationCoordinate2D(latitude: Double(coordenada[0])!, longitude: Double(coordenada[1])!)
